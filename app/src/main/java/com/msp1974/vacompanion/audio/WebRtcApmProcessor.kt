...
/**
 * WebRTC-based denoiser path for wake-word capture.
 *
 * This processor uses WebRTC NoiseSuppressor (NS) plus optional WebRTC VAD-based
 * non-speech attenuation.
 */
class WebRtcApmProcessor(
    private val enabled: Boolean,
    private val sampleRateHz: Int,
    private val channels: Int = 1,
    private val suppressionLevel: Int = 3, // 0..3, where 3 is most aggressive
    ...
    fun process(samples: ShortArray): ShortArray {
        ...
        // Complete one frame with pending samples from prior call.
        if (pendingSize > 0) {
            ...
            if (pendingSize + take == frameSamples) {
                if (!processOneFrame(inputFrame, outputFrame)) {
                    return samples
                }
                val frameGain = resolveFrameGateGain(outputFrame)

                // Only copy the portion that belongs to this call's samples.
                copyWithGain(outputFrame, pendingSize, output, 0, take, frameGain)
                outputOffset += take
                sampleOffset += take
                pendingSize = 0
            } else {
                // Still not enough for one frame; passthrough and keep buffering.
                System.arraycopy(samples, 0, output, 0, samples.size)
                System.arraycopy(samples, sampleOffset, pendingFrame, pendingSize, take)
                pendingSize += take
                return output
            }
        }

        // Process full frames from this call.
        ...
        // Trailing partial frame: passthrough now, keep raw samples for next call.
    ...
    private fun copyWithGain(
        ...
        val effectiveGain = (postGain * frameGain)
            .coerceIn(0f, 12f)
        ...
        for (i in 0 until count) {
            val value = (src[srcOffset + i].toFloat() * effectiveGain)
                .coerceIn(Short.MIN_VALUE.toFloat(), Short.MAX_VALUE.toFloat())
            dst[dstOffset + i] = value.roundToInt().toShort()
        }
    }
    ...
    private fun resolveFrameGateGain(frame: ShortArray): Float {
        ...
        return try {
            ...
            if (vad < 0) {
                // Disable VAD stage if runtime rejects the frame/format.
                vadActive = false
                if (vadHandle != 0L) {
                ...
                val isSpeech = vad > 0
                if (isSpeech) {
                    vadHangover = speechHangoverFrames
                }
                val inSpeechWindow = isSpeech || vadHangover > 0
                if (!isSpeech && vadHangover > 0) {
                    vadHangover--
                }
                if (inSpeechWindow) 1.0f else nonSpeechGain
            }
        } catch (t: Throwable) {
            // Keep pipeline alive if VAD stage fails unexpectedly.
            vadActive = false
            if (vadHandle != 0L) {
                runCatching { vadNative.nativeDestroy(vadHandle) }
                vadHandle = 0L
    ...
    override fun close() {
        if (handle != 0L) {
            runCatching { native.nativeDestroy(handle) }
        }
        if (vadHandle != 0L) {
            runCatching { vadNative.nativeDestroy(vadHandle) }
        }
